...
class CoreDataManager{
     ...
     */
    func getItemWithId(itemId: UUID) throws -> Item? {
        // Предикат для посика по id
        let request: NSFetchRequest<Item> = Item.fetchRequest()
        request.predicate = NSPredicate(format: "itemId=%@", (itemId.uuidString))
